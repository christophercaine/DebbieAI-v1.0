...
/**
 * Job Site Video Engine — Complete Video Pipeline
 * Built on FFmpegKit. Works on all Android devices.
 */
class JobSiteVideoEngine(private val cacheDir: File) {
    ...
    suspend fun processWalkthrough(inputPath: String, outputPath: String) {
        ...
        execute("-i \"$inputPath\" -vf vidstabtransform=smoothing=30:input=\"$transformFile\" \"$outputPath\"")
    }

    suspend fun createTimelapse(imageFolder: String, output: String, fps: Int = 8) {
        execute("-framerate $fps -pattern_type glob -i \"$imageFolder/*.jpg\" -c:v libx264 -pix_fmt yuv420p -crf 23 \"$output\"")
    }
    ...
    private suspend fun createSlideshow(images: List<String>, output: String, secPerFrame: Int) {
        ...
        execute("-f concat -safe 0 -i \"${listFile.absolutePath}\" -vf \"scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2\" -c:v libx264 -pix_fmt yuv420p \"$output\"")
    }

    private suspend fun addTextOverlay(input: String, text: String, output: String, x: Int, y: Int, fontSize: Int) {
        execute("-i \"$input\" -vf \"drawtext=text='$text':x=$x:y=$y:fontsize=$fontSize:fontcolor=white:shadowcolor=black:shadowx=3:shadowy=3\" \"$output\"")
    }
    ...
    private suspend fun addProjectBranding(input: String, project: String, company: String, output: String) {
        execute("-i \"$input\" -vf \"drawtext=text='$company':x=20:y=20:fontsize=32:fontcolor=white:shadowcolor=black:shadowx=2:shadowy=2,drawtext=text='$project':x=20:y=60:fontsize=22:fontcolor=yellow:shadowcolor=black:shadowx=2:shadowy=2\" \"$output\"")
    }
