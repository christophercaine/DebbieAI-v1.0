...
class JobSitePhotoEngine(
    ...
    suspend fun analyzePhoto(
        ...
        filePath: String,
        projectId: String
    ): JobSitePhoto = withContext(Dispatchers.IO) {
        ...
        JobSitePhoto(
            ...
            extractedText = ocrText.lines().filter { it.isNotBlank() },
            photoType = photoType
        )
    }

    // Auto-detect if this is a before/after/damage/material photo
    private fun classifyPhotoType(
        detections: List<Detection>, ocrText: String
    ): PhotoType {
